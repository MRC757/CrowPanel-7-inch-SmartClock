...
//
// Symbols fetched: ^GSPC (S&P 500), ^DJI (DOW), VYMI, VYM, GC=F (Gold), SI=F (Silver)
// One shared WiFiClientSecure is reused for all symbols (HTTP/1.1 keep-alive);
// only 1 TLS handshake per batch instead of 6.
// ─────────────────────────────────────────────────────────────────────────────
#include <Arduino.h>
