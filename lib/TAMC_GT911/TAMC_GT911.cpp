...
void TAMC_GT911::begin(uint8_t _addr) {
  addr = _addr;
  // Wire.begin() intentionally removed: setup() initializes Wire before calling
  // begin().  Calling Wire.begin() a second time on ESP32-S3 can leave a GDMA
  // channel allocated without a transfer, which prevents gfx.init() from
  // acquiring the GDMA channel it needs for the LCD_CAM DMA engine.
  reset();
  Serial.printf("[GT911] Touch controller ready at 0x%02X\n", addr);
}
void TAMC_GT911::reset() {
  // Read the GT911 config block into configBuf.
  // ESP32 Wire uses dynamic heap allocation (no 128-byte AVR limit), so all
  // 185 bytes are received cleanly and the I2C bus remains in good state.
  // This read is required: without it the Panel_RGB DMA produces no output
  // (empirically confirmed on CrowPanel 7" v1.3).
  readBlockData(configBuf, GT911_CONFIG_START, GT911_CONFIG_SIZE);
  // Write the config back with a valid checksum and set CONFIG_FRESH=1 so the
  // GT911 reloads its configuration and enters active touch scanning mode.
  // Safe to call now because configBuf holds the chip's own valid config.
